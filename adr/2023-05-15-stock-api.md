...
The stock handling in Cicada 6 is currently not very flexible and does not support many common use cases. 
...
We have a way to disable the stock handling behavior of Cicada.
...
### Update `\Cicada\Core\Content\Product\DataAbstractionLayer\ProductIndexer`

We update `\Cicada\Core\Content\Product\DataAbstractionLayer\ProductIndexer` to depend on `\Cicada\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage` as well as `Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdater`.

If `STOCK_HANDLING` is enabled then we call the `\Cicada\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage::index` method with the IDs of the product which have changed.

Otherwise, we call `Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdater::update`

### Deprecate `Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdater`

It will be removed with 6.6.

### Introduce `Cicada\Core\Content\Product\Stock\OrderStockSubscriber`

We introduce a new subscriber which listens to the various required events and interacts with `\Cicada\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage` via it's new API (`alter`).

All of Cicada's internal business rules for handling stock are located in this subscriber.

The subscriber listens to various events and calls `\Cicada\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage::alter` with the appropriate changesets for the following scenarios:
...
It is possible to disable Cicada's internal stock handling by setting the configuration `cicada.stock.enable_stock_management` to false.

### Introduce new core stock storage implementation.

We introduce a new implementation of `\Cicada\Core\Content\Product\DataAbstractionLayer\AbstractStockStorage` for managing the stock levels. It is responsible for incrementing/decrementing stock values based on the provided changesets.
...
* `available_stock` is calculated from the `stock` value minus open order quantities. This calculation is preformed in `Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdater::updateAvailableStockAndSales`. It is slow because the `SUM` may run over millions of rows.
...
* \Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdate\AbstractStockUpdateFilter
* \Cicada\Tests\Unit\Core\Content\Product\DataAbstractionLayer\StockUpdate\TestStockUpdateFilter
* \Cicada\Commercial\MultiWarehouse\Domain\Order\ExcludeMultiWarehouseStockUpdateFilter
* \Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdate\StockUpdateFilterProvider

### `ProductDefinition` updates

In Cicada version 6.6 or if the `STOCK_HANDLING` feature flag is enabled:
...
We update the various locations in Cicada where stock is loaded and augment the product with any stock information that is loaded from the stock storage.

This includes:

* \Cicada\Core\Content\Product\Subscriber\ProductSubscriber::salesChannelLoaded
* \Cicada\Core\Content\Product\SalesChannel\Detail\AvailableCombinationLoader::load
...
However, in order to support this API, we must update `\Cicada\Core\Content\Product\SalesChannel\Detail\AvailableCombinationLoader::load` because it currently does not pass along the `SalesChannelContext` which is necessary for `AbstractStockStorage::load`.
...
It is the role of `Cicada\Core\Content\Product\Stock\OrderStockSubscriber` to listen to the required cicada events for these scenarios and then interact with the stock storage implementation.
