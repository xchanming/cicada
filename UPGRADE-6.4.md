...
With the upcoming major release we are going to release a new XML-schema for Cicada Apps. In the new schema we remove two deprecations from the existing schema.

1. attribute `parent` for element `module` will be required.

   Please make sure that every of your admin modules has this attribute set like described in [our documentation](https://developer.cicada.com/docs/guides/plugins/apps/administration/add-custom-modules)
2. attribute `openNewTab` for element `action-button` will be removed.

    Make sure to remove the attribute `openNewTab` from your `action-button` elements in your `manifest.xml` and use ActionButtonResponses as described in our [documentation](https://developer.cicada.com/docs/guides/plugins/apps/administration/add-custom-action-button) instead.
3. Deprecation of `manifest-1.0.xsd`

    Update the `xsi:noNamespaceSchemaLocation` attribute of your `manifest` root element. to `https://raw.githubusercontent.com/cicada/platform/trunk/src/Core/Framework/App/Manifest/Schema/manifest-1.0.xsd`
...
We want to update the Storefront to Bootstrap v5 in the next major release of Cicada.
...
Because of the breaking changes inside Bootstrap v5 you will find several places with backward-compatibility code in the Cicada platform.
...
* **Bootstrap v5 OffCanvas**<br>
  Bootstrap v5 ships its own OffCanvas component. Cicada is still using its custom OffCanvas at the moment.
  It is planned to migrate the Cicada OffCanvas to the Bootstrap OffCanvas.
...
Cicada offers by default many assets like `theme`, all those assets have an own version strategy service in the di like `cicada.asset.theme.version_strategy`
...
Currently, Cicada needs to build the administration a database connection, to discover which plugins are active. To avoid that behaviour we have added a new `ComposerPluginLoader` which loads all information from the installed composer plugins.
...
use Cicada\Production\HttpKernel;
use Cicada\Production\Kernel;
...
storefront:
    theme:
        config_loader_id: Cicada\Storefront\Theme\ConfigLoader\StaticFileConfigLoader
        available_theme_provider: Cicada\Storefront\Theme\ConfigLoader\StaticFileAvailableThemeProvider
```
...
```xml
<service id="Cicada\Storefront\Controller\AccountPaymentController">
...
Storefront configuration provides a list of known parameters that fall in this category. You can overwrite or extend this list in your [bundle configuration](https://developer.cicada.com/docs/v/v6.4.0/guides/hosting/infrastructure/filesystem#configuration).   
```
storefront:
    http_cache:
...
We've also added support for PHP 8.0. While Cicada is de-facto ready for PHP 8.0,
...
The PHP extension `sodium` is now a requirement for Cicada 6.4.0.0.

---

## Composer 2
With Cicada 6.4 we are now requiring the `composer-runtime-api` with version 2.0.
These means that Cicada is now only installable with Composer 2.
...
Corresponding to the semantic versioning strategy of Cicada, we changed the API versioning to match the major versions of Cicada. 
...
With Cicada **6.3.5.0** we already made the new URL pattern available as an additional alias. 
...
### Detecting the current used Cicada / API
Of course, it is still important for an external service to know, which version of Cicada is used. 
Therefore we added a new information endpoint, which provides this information. 
The new endpoint is also available with Cicada **6.3.5.0**, so you can switch to this pattern in the 6.3 major cycle.
...
### API expectations
To have the version within the URL pattern offered the advantage of telling Cicada which version requirement you expect with the request. 
...
```http request
GET /api/test
sw-expect-packages: cicada/core:~6.4,swag/paypal:*
```

This example expects that the Cicada version is at least 6.4, and the PayPal extension is installed in any version. 
...
You can still remove unwanted fields from the response by using the `includes` property to [reduce the output](https://cicada.gitbook.io/docs/guides/integrations-api/general-concepts/search-criteria#includes-apialias).
...
See [the commit on GitHub](https://github.com/cicada/platform/commit/656c82d5232c87b75e1d6b42bd6493d674807791) for details.

---

## Changed the loading of storefront SCSS files in extensions
Previously all Storefront relevant SCSS files (`*.scss`) of an extension have automatically been loaded and compiled by cicada when placed inside the directory `src/Resources/app/storefront/src/scss`.
...
If your plugin is already installed the cicada core migration will take care of adjusting the foreign key.
...
Therefore, calling `Cicada\Core\Framework\Context::disableCache` has no more effect.
...
Example:
```js
Cicada.Service('productStreamConditionService').addToEntityAllowList('product', 'yourProperty');
```
