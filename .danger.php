...
const COMPOSER_PACKAGE_EXCEPTIONS = [
    ...
    'strict' => [
        ...
        '^cicada\/conflicts$' => 'The cicada conflicts packages should be required in any version, so use `*` constraint',
        '^cicada\/core$' => 'The cicada core packages should be required in any version, so use `*` constraint, the version constraint will be automatically synced during the release process',
...
return (new Config())
    ...
    ->useRule(new Condition(
        ...
        [
            function (Context $context): void {
                if (str_contains($context->platform->pullRequest->body, '/cicada/6/product/platform/-/merge_requests/')) {
                    return;
                }
                ...
                foreach ($context->platform->pullRequest->getComments() as $comment) {
                    if (str_contains($comment->body, '/cicada/6/product/platform/-/merge_requests/')) {
                        $found = true;
                        break;
                    }
                }
    ...
    ->useRule(function (Context $context): void {
        ...
        if ($files->matches('*/cicada.yaml')->count() > 0) {
            $context->warning('You updated the cicada.yaml, please consider to update the config-schema.json');
        }
    })
    ->useRule(new Condition(
        function (Context $context) {
