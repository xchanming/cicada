...
async function fetchGithub(url: string, { headers = {}, method = "GET", body }: { headers?: Record<string, string>, body?: string, method?: string } = {}) {
    ...
    headers['User-Agent'] = 'Cicada Release Info Generator';
...
async function fetchVulnerabilitiesByDescription(list: Array<Vulnerability>, body: string) {
    ...
    for (let match of unique) {
        const json = await (await fetchGithub(`https://api.github.com/repos/cicada/cicada/security-advisories/${match}`)).json();
...
async function generateVersionInfo() {
    const json = await (await fetchGithub("https://api.github.com/repos/cicada/cicada/releases")).json();
    ...
    for (const release of json) {
        ...
        marked.use(baseUrl(`https://github.com/cicada/cicada/blob/${release.tag_name}/changelog`));
...
async function generateVersionListing() {
    let currentPage = 1
    const latestRelease = await (await fetchGithub("https://api.github.com/repos/cicada/cicada/releases/latest")).json();
    const versions = [];

    while (true) {
        const releases = await (await fetchGithub("https://api.github.com/repos/cicada/cicada/releases?per_page=100&page=" + currentPage)).json();
...
async function fetchVulnerabilities() {
    const json = await (await fetchGithub("https://api.github.com/repos/cicada/cicada/security-advisories?per_page=100&state=published")).json();
