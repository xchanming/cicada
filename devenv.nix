...
in {
  ...
  services.mysql = {
    ...
    initialDatabases = lib.mkDefault [{ name = "cicada"; }];
    ensureUsers = lib.mkDefault [
      {
        name = "cicada";
        password = "cicada";
        ensurePermissions = {
          "cicada.*" = "ALL PRIVILEGES";
          "cicada_test.*" = "ALL PRIVILEGES";
        };
      }
    ];
    settings = {
  ...
  env.DATABASE_URL = lib.mkDefault "mysql://root@localhost:3306/cicada";
  ...
  env.CYPRESS_dbUser = lib.mkDefault "cicada";
  env.CYPRESS_dbPassword = lib.mkDefault "cicada";
  env.CYPRESS_dbName = lib.mkDefault "cicada";
  ...
  scripts.build-updater.exec = ''
      ...
      mv src/WebInstaller/cicada-installer.phar.php shop/public/cicada-installer.phar.php
  '';

  scripts.watch-updater.exec = "${pkgs.watchexec}/bin/watchexec -i src/WebInstaller/cicada-installer.phar.php  -eyaml,php,js build-updater";
}
