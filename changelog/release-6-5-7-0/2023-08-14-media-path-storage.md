...
* Added `\Cicada\Core\Framework\Struct\StateAwareTrait::state`, which allows to scope state changes
* Added new `\Cicada\Core\Framework\DataAbstractionLayer\Indexing\PostUpdateIndexer` class, which allows to run indexer after the update process but exclude them from whole indexing processes.
