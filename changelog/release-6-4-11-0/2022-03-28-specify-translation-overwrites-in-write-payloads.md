...
---
# Core
* Changed `\Cicada\Core\Framework\DataAbstractionLayer\FieldSerializer\TranslatedFieldSerializer` to not overwrite values specified direct under `translations`.
* Changed `\Cicada\Core\Framework\DataAbstractionLayer\FieldSerializer\TranslationsAssociationFieldSerializer` so that translation values with iso-codes take precedence over values with language ids.
___
# Upgrade Information
