...
For this, the event `\Cicada\Core\Content\Product\Events\ProductListingCollectFilterEvent` was introduced, where every developer can specify the meta data for a filter. 
...
There are currently systems that have performance problems with the `\Cicada\Core\Framework\DataAbstractionLayer\Dbal\EntityForeignKeyResolver`.
...
* Removed deprecations:
    * Removed deprecated property `allowedLimits` and method `getAllowedLimits` in `Cicada\Core\Framework\DataAbstractionLayer\Search/RequestCriteriaBuilder.php`
    * Removed deprecated configuration `api.allowed_limits` in `src/Core/Framework/Resources/config/packages/cicada.yaml`
    * Removed class `Cicada\Core\Framework\DataAbstractionLayer\Exception\DisallowedLimitQueryException`
* Added `CloneBehavior $behavior` parameter to `\Cicada\Core\Framework\DataAbstractionLayer\EntityRepositoryInterface::clone`. This parameter will be introduced in 6.4.0.
    ...
    * After
    ...
    ```
* Changed status code of `\Cicada\Core\System\SalesChannel\NoContentResponse` from `200` to `204`.
* Added two new arguments `$package` and `$cacheClearer` to constructor of `\Cicada\Storefront\Theme\ThemeCompiler`.
* Replaced Symfony `asset:install` command with a Flysystem compatible own implementation
* Added new filesystem adapters `cicada.filesystem.theme`, `cicada.filesystem.asset` and `cicada.filesystem.sitemap`. They can be configured to use external storages for saving of theme contents, bundle assets or sitemap.
* Added new argument `$package` to constructor of `\Cicada\Core\Content\Sitemap\Service\SitemapLister`.
* Deprecated config `cicada.cdn.url`. Use `cicada.filesystem.public.url` instead.
* Added new scss variable `sw-asset-theme-url` which refers to the theme asset url.
* If you subscribed to one of the following `\Cicada\Storefront\Event\RouteRequest\RouteRequestEvent` events, you must now extend the provided criteria instead of adding the query to the request:
    * Before
    ```php
...
* Added `Criteria $criteria` parameter in store api routes. The parameter will be required in 6.4. At the moment, the parameter is commented out in the `*AbstractRoute`, but it is already passed. If you decorate on of the following routes, you have to change your sources as follows:
      * Affected routes:
          * `Cicada\Core\Checkout\Customer\SalesChannel\AbstractCustomerRoute`           
          * `Cicada\Core\Checkout\Order\SalesChannel\AbstractOrderRoute`                 
          * `Cicada\Core\Checkout\Payment\SalesChannel\AbstractPaymentMethodRoute`       
          * `Cicada\Core\Checkout\Shipping\SalesChannel\AbstractShippingMethodRoute`     
          * `Cicada\Core\Content\Category\SalesChannel\AbstractNavigationRoute`          
          * `Cicada\Core\Content\Product\SalesChannel\Listing/AbstractProductListingRoute`
          * `Cicada\Core\Content\Product\SalesChannel\Search/AbstractProductSearchRoute` 
          * `Cicada\Core\Content\Seo\SalesChannel\AbstractSeoUrlRoute`                   
          * `Cicada\Core\System\Currency\SalesChannel\AbstractCurrencyRoute`             
          * `Cicada\Core\System\Language\SalesChannel\AbstractLanguageRoute`             
          * `Cicada\Core\System\Salutation\SalesChannel\AbstractSalutationRoute`         
      * Sources before:
          ```
          /**
           * @Route("/store-api/v{version}/account/customer", name="store-api.account.customer", methods={"GET"})
...
* The behaviour when uninstalling a plugin has changed: `keepMigrations` now has the same value as `keepUserData` in `\Cicada\Core\Framework\Plugin\Context\UninstallContext` by default.
...
* We have moved the logic for loading the detail page to Store-Api routes. The following extension points have been adapted for this:
    * Some services and struct classes moved from the `Cicada\Storefront` domain to the `Cicada\Core` domain. The public api are still the same but if you decorated one of the following classes you have to change your `extends` expression and the `decorates` definition in your services.xml:
        * `\Cicada\Storefront\Page\Product\Configurator\AvailableCombinationLoader` => `Cicada\Core\Content\Product\SalesChannel\Detail\AvailableCombinationLoader`
        * `\Cicada\Storefront\Page\Product\Configurator\ProductPageConfiguratorLoader` => `Cicada\Core\Content\Product\SalesChannel\Detail\ProductConfiguratorLoader`
        * `\Cicada\Storefront\Page\Product\CrossSelling\CrossSellingLoader` => `\Cicada\Core\Content\Product\SalesChannel\CrossSelling\AbstractProductCrossSellingRoute`
        * `\Cicada\Storefront\Page\Product\ProductLoader` => `Cicada\Core\Content\Product\SalesChannel\Detail\ProductDetailRoute`
    * Usage of the `\Cicada\Storefront\Page\Product\ProductLoader` are no longer recommend. To fetch the product data of a single product, use the `Cicada\Core\Content\Product\SalesChannel\Detail\ProductDetailRoute`
        * With this deprecation we also deprecated the `\Cicada\Storefront\Page\Product\ProductLoaderCriteriaEvent`. 
            * If you have subscribed to this event to extend the product detail page, replace the event with `\Cicada\Storefront\Page\Product\ProductPageCriteriaEvent`
            * If you have subscribed to this event to extend the listing quick view, replace the event with `\Cicada\Storefront\Page\Product\QuickView\MinimalQuickViewPageCriteriaEvent`  
    * As with the services, we have also moved some events from the storefront to the core. The public API of the events is the same. The following events can be replaced 1:1:
        * `\Cicada\Storefront\Page\Product\CrossSelling\CrossSellingLoadedEvent` => `\Cicada\Core\Content\Product\Events\ProductCrossSellingsLoadedEvent` instead
        * `\Cicada\Storefront\Page\Product\CrossSelling\CrossSellingProductCriteriaEvent` => `\Cicada\Core\Content\Product\Events\ProductCrossSellingCriteriaEvent` instead
        * `\Cicada\Storefront\Page\Product\CrossSelling\CrossSellingProductListCriteriaEvent` => `\Cicada\Core\Content\Product\Events\ProductCrossSellingIdsCriteriaEvent` instead
        * `\Cicada\Storefront\Page\Product\CrossSelling\CrossSellingProductStreamCriteriaEvent` => `\Cicada\Core\Content\Product\Events\ProductCrossSellingStreamCriteriaEvent` instead
...
* Removed LanguageStore
    ...
    * Replace `languageStore.setCurrentId(this.languageId)` with `Cicada.State.commit('context/setApiLanguageId', languageId)`
    * Replace `languageStore.getCurrentId()` with `Cicada.Context.api.languageId`
    * Replace `getCurrentLanguage` with the Repository
    * Removed `getLanguageStore`
    * Replace `languageStore.systemLanguageId` with `Cicada.Context.api.systemLanguageId`
    * Replace `languageStore.currentLanguageId` with `Cicada.Context.api.languageId`
...
[See the updated documentation to how to configure all filesystems.](https://docs.cicada.com/en/cicada-platform-dev-en/how-to/use-s3-datastorage)
...
These assets are prefixed in dependency injection with `cicada.asset.{ADAPTER_NAME}`:  
*  `cicada.asset.public`
*  `cicada.asset.theme`
*  `cicada.asset.asset`
...
$publicAsset = $container->get('cicada.asset.public');
