...
download_store_plugin() {
    ...
    local download_info_url="https://api.cicada.com/swplatform/pluginfiles/${technical_name}?domain=sw6.fk6.test.cicada.in&language=en-GB&cicadaVersion=${PLATFORM_VERSION}"
    local info=$(curl \
        --location --silent \
    ...
    plugin_name="$(jq '.extra["cicada-plugin-class"]' < "${download_dir}/composer.json" | sed 's/"//g' | tr '\\' "\n" | tail -n 1)"
...
activate_plugin() {
    ...
    local plugin_name="$(jq '.extra["cicada-plugin-class"]' < "${path}/composer.json" | sed 's/"//g' | tr '\\' "\n" | tail -n 1)"
