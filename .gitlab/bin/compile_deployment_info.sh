...
current_major_alias() {
  ...
  curl -fsSL "https://releases.cicada.com/changelog/index.json" |
...
custom_version_extensions() {
  ...
  git clone --depth=1 "https://gitlab-ci-token:${CI_JOB_TOKEN}@gitlab.cicada.com/cicada/6/product/saas.git" "${tmpdir}"
...
commit_date() {
  ...
  export GITLAB_HOST="gitlab.cicada.com"
...
<ul>
<li><span>cicada/platform: <b>$(commit_date "cicada%2F6%2Fproduct%2Fplatform" "${deployment_branch_name_url_encoded}")</b>(Pipeline: $(get_latest_pipeline_web_url "cicada%2F6%2Fproduct%2Fplatform" "${deployment_branch_name_url_encoded}"))</span></li>
<li><span>cicada/commercial: <b>$(commit_date "cicada%2F6%2Fproduct%2Fcommercial" "${deployment_branch_name_url_encoded}")</b>(Pipeline: $(get_latest_pipeline_web_url "cicada%2F6%2Fproduct%2Fcommercial" "${deployment_branch_name_url_encoded}"))</span></li>
<li><span>swag/saas-rufus: <b>$(commit_date "cicada%2F6%2Fproduct%2Frufus" "${deployment_branch_name_url_encoded}")</b>(Pipeline: $(get_latest_pipeline_web_url "cicada%2F6%2Fproduct%2Frufus" "${deployment_branch_name_url_encoded}"))</span></li>
</ul>
</p>
<hr/>
EOF
...
deployment_env() {
  ...
  local gitlab_mr_assignees="cicadabot"
