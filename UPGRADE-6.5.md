...
You can find more details about the cache rework in the [cicada/cicada discussions](https://github.com/cicada/cicada/discussions/3299)
...
As foundation, we use `\Cicada\Core\Content\Media\Core\Application\AbstractMediaPathStrategy` as base class and dependency injection service id:
...
use Cicada\Core\Content\Media\Core\Application\AbstractMediaPathStrategy;use Cicada\Core\Content\Media\Core\Application\MediaLocationBuilder;
...
If you have previously decorated `\Cicada\Core\Content\Product\DataAbstractionLayer\StockUpdater` you must refactor your code. Depending on what you want to accomplish you have two options:
...
Similarly, if you consume `\Cicada\Core\Content\Product\SalesChannel\Detail\AbstractAvailableCombinationLoader` then you will need to adjust your code, to pass in `\Cicada\Core\System\SalesChannel\SalesChannelContext`.
...
## Disabling Cicada's stock management system
...
With [4457](https://github.com/cicada/cicada/issues/4457) we fixed an issue with duplicated theme images on `system:update` and `theme:refresh`.
...
This comment on github could also be helpful: [github how to clean theme media](https://github.com/cicada/platform/discussions/3254#discussioncomment-6666360)
...
Cicada now uses Symfony version 6.3, please make sure your plugins are compatible.
...
* Replace any old icon your integration uses with its successor. A mapping can be found [here](https://github.com/cicada/cicada/blob/v6.4.20.2/src/Administration/Resources/app/administration/src/app/component/base/sw-icon/legacy-icon-mapping.js).
...
* action `setAppModules` in `src/app/state/cicada-apps.store.ts` is removed
* action `setAppModules` in `src/app/state/cicada-apps.store.ts` is removed
...
Cicada now uses symfony components in version 6.2, please make sure your plugins are compatible.
...
All type hints from EntityRepositoryInterface should be changed to EntityRepository, you can use [rector](https://github.com/FriendsOfCicada/cicada-rector) for that.
...
* In the next major, the flow actions aren't executed over the symfony events anymore; we'll remove the dependence from `EventSubscriberInterface` in `Cicada\Core\Content\Flow\Dispatching\Action\FlowAction`.
* In the next major, the flow actions aren't executed via symfony events anymore;
  we'll remove the dependency from `EventSubscriberInterface` in `Cicada\Core\Content\Flow\Dispatching\Action\FlowAction`.
...
We removed `\Cicada\Core\Framework\Event\FlowEvent`, since Flow Actions aren't executed via symfony's event system anymore.
...
If you've implemented a custom value pattern please use the abstract class `Cicada\Core\System\NumberRange\ValueGenerator\Pattern\AbstractValueGenerator`.
...
If you've implemented a custom increment storage please use the abstract class `Cicada\Core\System\NumberRange\ValueGenerator\Pattern\IncrementStorage\AbstractIncrementStorage`.
Before:
...
Below you can find a migration overview of the effected areas in the Cicada platform.
...
With the update to Bootstrap v5, the jQuery dependency will be removed from the cicada platform.
...
This is the recommended method for all apps/themes which don't have control over the Cicada environment in which they're running in.
...
* This block is not deprecated and can be used in the long term beyond the next major version of cicada.
...
* After executing `bin/console asset:install`, you can reference the file using the `assset()` function. See also: https://developer.cicada.com/docs/guides/plugins/plugins/storefront/add-custom-assets
...
previously bundled JavaScript which was created with Cicada 6.4.x is not compatible with Cicada 6.5.
...
With the upcoming major release, we're going to release a new XML-schema for Cicada Apps.
...
1. attribute `parent` for element `module` will be required.
   ...
   like described in [our documentation](https://developer.cicada.com/docs/guides/plugins/apps/administration/add-custom-modules)
...
3. Deprecation of `manifest-1.0.xsd`
   ...
   Update the `xsi:noNamespaceSchemaLocation` attribute of your `manifest` root element to `https://raw.githubusercontent.com/cicada/platform/trunk/src/Core/Framework/App/Manifest/Schema/manifest-2.0.xsd`
