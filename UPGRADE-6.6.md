...
## Vue 3 upgrade
We upgraded to Vue 3, for details check out our [upgrade guide](https://developer.cicada.com/docs/resources/references/upgrades/administration/vue3.html#vue-3-upgrade).
...
Cross-compatibility with older cicada versions is not possible because we upgraded the build system, e.g. admin extensions built for cicada 6.6 with webpack 5 will not work with cicada 6.5 (and webpack 4) or lower.
When you want to have a single version of your admin extension, you should consider switching to the [`meteor-admin-sdk`](https://cicada.github.io/meteor-admin-sdk/) as that lets you control your extensions runtime environment.
...
**Note:** This already works for older cicada versions and is considered best practice.
...
* Removed `\Cicada\Core\Content\ProductExport\Exception\EmptyExportException` use `\Cicada\Core\Content\ProductExport\ProductExportException::productExportNotFound` instead
...
It is replaced by `\Cicada\Core\Framework\DataAbstractionLayer\Event\EntityDeleteEvent` with the same API.

You should use `\Cicada\Core\Framework\DataAbstractionLayer\Event\EntityDeleteEvent` instead, only the class name changed.
...
Removed `\Cicada\Core\Content\Product\Cart\ProductLineItemFactory`, use `\Cicada\Core\Checkout\Cart\LineItemFactoryHandler\ProductLineItemFactory` instead.
...
Removed the following exception classes:
* `\Cicada\Core\Content\ProductExport\Exception\RenderFooterException`
* `\Cicada\Core\Content\ProductExport\Exception\RenderHeaderException`
* `\Cicada\Core\Content\ProductExport\Exception\RenderProductException`
...
## Removal of `blacklistIds` and `whitelistIds` in  `\Cicada\Core\Content\Product\ProductEntity`
Two properties `blacklistIds` and `whitelistIds` were removed without replacement

## Replace `@cicada-ag/admin-extension-sdk` with `@cicada-ag/meteor-admin-sdk`

### Before
```json
{
    "dependencies": {
        "@cicada-ag/admin-extension-sdk": "^3.0.14"
    }
}
```
...
{
    "dependencies": {
        "@cicada-ag/meteor-admin-sdk": "^3.0.16"
    }
}
```

## Update `@cicada-ag/meteor-admin-sdk` to `^4.0.0`

### Before
```json
{
    "dependencies": {
        "@cicada-ag/meteor-admin-sdk": "^3.0.17"
    }
}
```
...
{
    "dependencies": {
        "@cicada-ag/meteor-admin-sdk": "^4.0.0"
    }
}
```
...
<div
    v-tooltip="{
        message: 'For more information click <a href=\"https://cicada.com\">here</a>.',
    }"
</div>
```
...
<div
    v-tooltip="{
        message: 'For more information visit cicada.com',
    }"
</div>
```
