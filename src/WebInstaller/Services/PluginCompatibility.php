...
/**
 * Any plugins which  execute composer commands @see \Cicada\Commercial\SwagCommercial::executeComposerCommands
 * must be removed from the project `composer.json` file when their cicada dependency constraint is not valid with the updated Cicada version.
 *
 * Eg: Cicada is being updated to 6.6. Plugin `cicada/commercial` depends on `cicada/core` with a constraint like "^6.5".
...
class PluginCompatibility
{
    ...
    public function __construct(
        ...
        private string $cicadaUpgradeVersion,
    ) {
    ...
    {
        $cicadaPlugins = $this->getInstalledPackagesByType('cicada-platform-plugin');
        ...
        $customComposerPlugins = array_filter($cicadaPlugins, function (string $plugin) {
        ...
        $nonCompatible = array_filter(
            ...
            fn ($plugin) => !$this->isPluginCompatible($plugin, $this->cicadaUpgradeVersion),
        );
    ...
    private function isPluginCompatible(string $plugin, string $cicadaVersion): bool
    {
        $pluginCompatible = true;

        foreach ($this->getRequires($plugin) as $packageDep => $constraint) {
            if (\in_array($packageDep, self::CICADA_PACKAGES, true)) {
                $satisfies = Semver::satisfies($cicadaVersion, $constraint);
