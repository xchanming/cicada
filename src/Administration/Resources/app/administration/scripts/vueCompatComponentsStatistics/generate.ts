...
interface componentInfo {
    usesCicadaCompatConfig?: boolean,
    path: string,
...
for (const [componentName, componentInfo] of componentMapEntries) {
    ...
    if (file) {
        ...
        componentMap[componentName].usesCicadaCompatConfig = fileContent.includes('compatConfig: Cicada.compatConfig');
    }
...
const readableFileStructure = Object.entries(componentMap).reduce<{
    usesCicadaCompatConfig: string[],
    noCicadaCompatConfig: string[],
}>((acc, [componentName, componentInfo]) => {
    if (componentInfo.usesCicadaCompatConfig) {
        acc.usesCicadaCompatConfig.push(componentName);
    } else {
        acc.noCicadaCompatConfig.push(componentName);
    }

    return acc;
}, {
    usesCicadaCompatConfig: [],
    noCicadaCompatConfig: [],
});
