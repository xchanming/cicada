...
describe('module/sw-settings-cicada-updates/page/sw-settings-cicada-updates-wizard', () => {
    let wrapper;

    beforeEach(async () => {
        Cicada.Application.view.deleteReactive = () => {};
        wrapper = mount(
            await wrapTestComponent('sw-settings-cicada-updates-wizard', {
                sync: true,
            }),
            {
                global: {
                        ...
                        updateService: {
                            ...
                            checkRequirements: () =>
                                Promise.resolve([
                                    ...
                                    {
                                        name: 'validCicadaLicense',
                                        result: false,
                                        message: 'validCicadaLicense',
                                        vars: [],
                                        extensions: [],
                                    },
                                ]),
                    ...
                    stubs: {
                        ...
                        'sw-settings-cicada-updates-requirements': await wrapTestComponent(
    ...
    it('deactivate plugins success', async () => {
        ...
        expect(redirectSpy).toHaveBeenCalledWith(`${Cicada.Context.api.basePath}/cicada-installer.phar.php`);
    });
    ...
    it('click on update button', async () => {
        ...
        await wrapper.get('.sw-settings-cicada-updates-wizard__start-update').trigger('click');
        await flushPromises();

        expect(wrapper.vm.updateModalShown).toBe(true);

        expect(wrapper.find('.sw-settings-cicada-updates-check__start-update').exists()).toBe(true);

        await wrapper.get('.sw-settings-cicada-updates-check__start-update-backup-checkbox input').setChecked(true);

        const redirectSpy = jest.fn();
        wrapper.vm.redirectToPage = redirectSpy;

        await wrapper.get('.sw-settings-cicada-updates-check__start-update-button').trigger('click');
