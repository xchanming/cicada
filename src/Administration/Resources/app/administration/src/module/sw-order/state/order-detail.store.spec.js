...
describe('src/module/sw-order/state/order-detail.store', () => {
    ...
    beforeAll(() => {
        Cicada.State.registerModule('swOrderDetail', orderDetailState);
    });

    beforeEach(() => {
        state = Cicada.State.get('swOrderDetail');
    });

    it('should be able to setOrder', () => {
        const newOrder = { id: 1, name: 'Test Order' };

        Cicada.State.commit('swOrderDetail/setOrder', newOrder);
    ...
    it('should be able to setLoading', () => {
        ...
        expect(Cicada.State.getters['swOrderDetail/isLoading']).toBe(true);
    });

    it('should be able to setSavedSuccessful', () => {
        Cicada.State.commit('swOrderDetail/setSavedSuccessful', true);
    ...
    it('should be able to setVersionContext', () => {
        ...
        Cicada.State.commit('swOrderDetail/setVersionContext', versionContext);

        expect(state.versionContext).toEqual(versionContext);
    });

    it('should be able to setEditing', () => {
        Cicada.State.commit('swOrderDetail/setEditing', true);

        expect(state.editing).toBe(true);
        expect(Cicada.State.getters['swOrderDetail/isEditing']).toBe(true);
    });
    ...
    it('should set order address ids when provided valid address info', () => {
        ...
        Cicada.State.commit('swOrderDetail/setOrderAddressIds', addressIdInfo);
    ...
    it('should update customerAddressId when orderAddressId and type match', () => {
        ...
        Cicada.State.commit('swOrderDetail/setOrderAddressIds', initialAddressIdInfo);
        Cicada.State.commit('swOrderDetail/setOrderAddressIds', updatedAddressIdInfo);
    ...
    it('should remove order address id when orderAddressId equals customerAddressId and type match', () => {
        ...
        Cicada.State.commit('swOrderDetail/setOrderAddressIds', initialAddressIdInfo);
        Cicada.State.commit('swOrderDetail/setOrderAddressIds', removalAddressIdInfo);
