...
function getCollection(repository, entities = [], aggregations = defaultAggregations, total = 0) {
    return new EntityCollection(
        ...
        repository,
        Context.api,
        { isCicadaContext: true, page: 1, limit: 25 },
        entities,
        total,
        aggregations,
    );
...
describe('src/module/sw-settings-rule/page/sw-settings-rule-detail', () => {
    ...
    it.each([
        ...
        const apiLanguageId = Cicada.State.get('context').api.languageId;
        expect(Cicada.State.get('context').api.languageId).not.toBe('uuid1');
        ...
        expect(Cicada.State.get('context').api.languageId).toBe(abort ? apiLanguageId : 'uuid1');

        // cleanup
        Cicada.State.commit('context/setApiLanguageId', apiLanguageId);
    });
