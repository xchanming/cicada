...
test('Install a new Cicada instance.', { tag: '@Install' }, async ({ InstallPage }) => {
    ...
    await expect(page.locator('#import-finished:visible div').first())
        .toHaveText('Cicada 6 has been installed!', { timeout: 120000 });
    await page.getByRole('link', { name: 'Next' }).click();
    ...
    await page.getByLabel('Admin password:').fill('cicada');
