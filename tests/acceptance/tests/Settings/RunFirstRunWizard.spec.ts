...
test('Merchant is able to be guided through the First Run Wizard.', { tag: '@FirstRunWizard' }, async ({
    ...
    test.skip(InstanceMeta.features['V6_7_0_0'], 'This test is incompatible with V6_7_0_0. Ticket: https://cicada.atlassian.net/browse/NEXT-40160');
    ...
    // Cicada account part
    await ShopAdmin.expects(AdminFirstRunWizard.cicadaAccountHeader).toBeVisible();
    ...
    // Cicada store part
    await ShopAdmin.expects(AdminFirstRunWizard.cicadaStoreHeader).toBeVisible({ timeout: 120000 });
