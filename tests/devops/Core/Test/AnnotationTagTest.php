...
class AnnotationTagTest extends TestCase
{
    ...
    private function getCicadaVersion(): string
    {
        ...
        if (!preg_match('/^\d+\.\d+[.-].*$/', $cicadaVersion)) {
            // this will only check the syntax of the deprecated tags. The real test happens in the prod pipeline

            $matches = [];
            preg_match('/(\d+\.\d+)\..*/', Kernel::CICADA_FALLBACK_VERSION, $matches);
            static::assertArrayHasKey(1, $matches);

            // get major version from Kernel::CICADA_FALLBACK_VERSION
            $cicadaVersion = $matches[1] . '.0';
        }
