...
class ErrorResponseFactoryTest extends TestCase
{
    ...
    public function testStackTraceForExceptionInDebugMode(\Exception $exception): void
    {
        ...
        $stack = $exception instanceof CicadaHttpException
            ? $data['errors'][0]['trace']
    ...
    public static function getResponseFromExceptionProvider(): \Generator
    {
        ...
        yield 'cicada http exception' => [new PageNotFoundException($message)];
    }
    ...
    public function testItUnwindsCicadaHttpException(): void
    {
        $params = [
            'paramOne' => '1',
            'paramTwo' => '2',
        ...
        $simpleHttpException = new SimpleCicadaHttpException($params);
        $errorResponseFactory = new ErrorResponseFactory();
        ...
        static::assertEquals([
            'errors' => [
                [
                    'code' => SimpleCicadaHttpException::EXCEPTION_CODE,
                    'status' => '418',
    ...
    public function testWriteExceptionConvertsCicadaHttpExceptionCorrectly(): void
    {
        $errorResponseFactory = new ErrorResponseFactory();

        $cicadaHttpException = new SimpleCicadaHttpException(['paramOne' => 1, 'paramTwo' => 2]);
        $errorFromWrite = $errorResponseFactory->getResponseFromException((new WriteException())->add($cicadaHttpException));
        $errorRaw = $errorResponseFactory->getResponseFromException($cicadaHttpException);
    ...
    {
        $simpleCicadaHttpException = new SimpleCicadaHttpException(['paramOne' => 1, 'paramTwo' => 2]);
        $writeException = (new WriteException())
            ->add(
                (new WriteException())
                    ->add($simpleCicadaHttpException)
                    ->add($simpleCicadaHttpException)
            )->add(
                (new WriteException())
                    ->add($simpleCicadaHttpException)
        ...
        $convertedCicadaHttpException = $errorResponseFactory->getErrorsFromException($simpleCicadaHttpException)[0];

        $responseBody = json_decode((string) $response->getContent(), true, 512, \JSON_THROW_ON_ERROR);

        static::assertCount(4, $responseBody['errors']);
        static::assertEquals([
            $convertedCicadaHttpException,
            $convertedCicadaHttpException,
            $convertedCicadaHttpException,
            $convertedCicadaHttpException,
        ], $responseBody['errors']);
    ...
    public function testResourceValueShouldNotThrow(): void
    {
        ...
        try {
            ...
            $exception = new SimpleCicadaHttpException([
                'normal' => 'value',
 ...
 * @internal
 */
class SimpleCicadaHttpException extends CicadaHttpException
{
