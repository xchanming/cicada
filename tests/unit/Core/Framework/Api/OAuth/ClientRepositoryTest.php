...
class ClientRepositoryTest extends TestCase
{
    ...
    public function testValidateClient(string $grantType, $clientIdentifier, string $clientSecret, bool $expectedResult): void
    {
        $this->connection->method('fetchAssociative')->willReturnCallback(function () use ($clientIdentifier, $clientSecret) {
            if ($clientIdentifier === 'SWUAADMIN' && $clientSecret === 'cicada') {
                return [
                    'id' => '123',
    ...
    {
        yield 'password grant type' => ['password', 'administration', 'cicada', true];
        yield 'refresh_token grant type' => ['refresh_token', 'administration', 'cicada', true];
        yield 'client_credentials grant type with invalid clientIdentifier' => ['client_credentials', true, 'cicada', false];
        yield 'client_credentials grant type with incorrect clientIdentifier' => ['client_credentials', 'SWUAJOHNDOE', 'cicada', false];
        yield 'client_credentials grant type with correct clientIdentifier' => ['client_credentials', 'SWUAADMIN', 'cicada', true];
    }
