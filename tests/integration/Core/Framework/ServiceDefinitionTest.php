...
class ServiceDefinitionTest extends TestCase
{
    ...
    public function testEverythingIsInstantiatable(): void
    {
        $excludes = [
            '_dummy_es_env_usage',
            'kernel.bundles',
            'cicada.cache.invalidator.storage.redis', // causes redis connect
            'cicada.cache.invalidator.storage.redis_adapter',  // causes redis connect
        ];
    ...
    private function checkServiceParameterOrder(string $content): array
    {
        ...
        // only continue if a Cicada service definition doesn't start with class followed by id
        if (!$result || empty($matches)) {
            return [];
        }
