...
class PluginServiceTest extends TestCase
{
    ...
    public function testRefreshPluginWithoutExtraLabelProperty(): void
    {
        ...
        $composerJsonException = $errors->filter(fn (CicadaHttpException $error) => $error instanceof PluginComposerJsonInvalidException);

        static::assertNotEmpty($composerJsonException);

        $errorFound = false;
        $errorString = 'Plugin composer.json has invalid "type" (must be "cicada-platform-plugin"), or invalid "extra/cicada-plugin-class" value, or missing extra.label property';
    ...
    private function assertPluginMetaInformation(PluginEntity $plugin): void
    {
        ...
        static::assertSame('cicada AG', $plugin->getAuthor());
        static::assertSame('(c) by cicada AG', $plugin->getCopyright());
